...
export function deflate_decode(base_compressed) {
	const retptr = 8;
	const ret = wasm.deflate_decode(retptr, passArray8ToWasm(base_compressed), WASM_VECTOR_LEN);
...
export function deflate_encode(base_raw) {
	const retptr = 8;
	const ret = wasm.deflate_encode(retptr, passArray8ToWasm(base_raw), WASM_VECTOR_LEN);
	const memi32 = getInt32Memory();
	const v0 = getArrayU8FromWasm(memi32[retptr / 4 + 0], memi32[retptr / 4 + 1]).slice();
	wasm.__wbindgen_free(memi32[retptr / 4 + 0], memi32[retptr / 4 + 1] * 1);
	return v0;
